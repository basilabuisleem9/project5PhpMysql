	...
	<div class="row">
		...
		<main class="col-md-9">
			...
			<header class="border-bottom mb-4 pb-3">
				<div class="form-inline">
					...
					if(isset($_GET['search_word'])) {
						...
						$query = "SELECT COUNT(product_id) FROM proudcts where catID='$cat_id' AND 'product_name' LIKE '%$keyword%'";
						//echo $query;
					} else {
			...
			<div class="row">
				...
				if (isset($_GET['search_word'])) {
					$keyword = $_GET['search_word'];
					$query = "SELECT COUNT(product_id) FROM proudcts where catID='$cat_id' AND price BETWEEN $min AND $max AND `product_name` LIKE '%$keyword%'";
					
				} else {
					$query = "SELECT COUNT(product_id) FROM proudcts where catID='$cat_id' AND catID='$cat_id' AND price BETWEEN $min AND $max";
				}

				$productsQuery = null;
				if (isset($_GET['search_word'])) {
					$keyword = $_GET['search_word'];
					$productsQuery = "SELECT * FROM proudcts where catID='$cat_id' AND price BETWEEN $min AND $max AND `product_name` LIKE '%$keyword%'";
				} else {
					$productsQuery = "SELECT * FROM proudcts where catID='$cat_id' AND catID='$cat_id' AND price BETWEEN $min AND $max";
				}
							...
							<figure class="card card-product-grid">
								...
								<figcaption class="info-wrap">
									...
									<form method="POST" action="">
										...
										<button class="btn btn-block btn-dark btn-lg mt-4" name="add_item" type="submit">Add to cart </button>
									</form>
								</figcaption>
							</figure>
						</div> <!-- col.// -->
